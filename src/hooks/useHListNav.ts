...
export function useHListNav({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (scroller && element) {
        ...
        scrollIntoViewH(scroller, element, 'auto');
      }
    }, 50);
  ...
  function handleKeyPress(ev: KeyboardEvent): void {
    ...
    if (data && priority !== data.priority) {
      ...
      if (!scroller) {
        return;
      } else if (ev.key === 'ArrowLeft' && scroller.scrollTop > 0) {
        // Scroll up
        scroller.scrollBy({
    ...
    let newIndex = ev.key === 'ArrowLeft' ? index - 1 : index + 1;
    if (index === -1 && ev.key === 'ArrowLeft') {
    ...
    if (data.results[newIndex]) {
      scrollIntoViewH(scroller, data.results[newIndex].element as HTMLElement);
    } else if (ev.key === 'ArrowLeft' && scroller.scrollTop > 0) {
      // Scroll up
      scroller.scrollBy({
